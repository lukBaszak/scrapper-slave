...
class WebScrapper:
    ...
    def scrap_url(self):
        ...
        views = offer_bottombar__items.find_element_by_xpath('//span[@class="offer-bottombar__counter"]/strong').text

        post_data = {"url": blob,"title": title, "price": price, "quality": quality, "created": create_date, "views": views}
        json_post_data = json.dumps(post_data)

        requests.post(f"{self.parent_ip}/scrapper/add_data", json=json_post_data)
